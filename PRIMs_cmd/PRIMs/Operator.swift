 ...
 */
    ...
    */
    ...
    func findOperator() -> Bool {
        ...
        //Command that save (to write in the output file in batch mode) the operators retrieved
        if model.dm.goalSelection && model.batchMode {
            let timeString = String(format:"%.2f", model.time)
            model.retrievedOp.append(timeString+"_"+opRetrieved!.name)
        }
