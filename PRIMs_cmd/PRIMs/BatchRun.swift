...
class BatchRun {
    ...
    init(script: String, mainModel: Model, outputFile: URL, controller: MainViewController, directory: URL, fileName: String) {
...
//                          output += "\(line.firings)
                        ...
                        if self.model.dm.goalSelection && self.model.batchMode {
                            var totalTrials: Int = 0
                            var nextTask = self.model.tasks.randomElement()
                            
                            let _threshold: Int = 20
                            let _distance: Int = 50
                            let nextBL = nextTask!.accuracyDerivative(threshold: _threshold, distance: _distance)
                            //print("\(nextTask!.name) \(nextBL)")
                            for task in self.model.tasks {
                                totalTrials += task.goalHistory.count
                                let potentialBL = task.accuracyDerivative(threshold: _threshold, distance: _distance)
                                //print("\(task.name) \(potentialBL)")
                                if  potentialBL > nextBL {
                                    nextTask = task
                                }
                            }
                            //print("finally: \(nextTask!.name) \(nextTask!.accuracyDerivative(threshold: _threshold, distance: _distance))")
                            if totalTrials < 10000 {
