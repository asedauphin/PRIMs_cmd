...
//      let dl = DataLine(eventType: "trial-start", eventParameter1: "void", eventParameter2: "void", eventParameter3: "void", inputParameters: model!.scenario.inputMappingForTrace, time:model!.startTime, firings: model!.firings)
        let dl = DataLine(eventType: "trial-start", time: model!.startTime, goalBaselevelActivation: slot4!, reachGoal: false, retrievedOperators: "void")
...
//    model!.running = false
    ...
    if model!.running && model!.batchMode {
        var slot4: Double?
        if let goal = model!.buffers["goal"] {
        for (_,value) in goal.slotvals {
            switch value {
...
//        let dl = DataLine(eventType: "trial-end", eventParameter1: "success", eventParameter2: "void", eventParameter3: "void", inputParameters: model!.scenario.inputMappingForTrace, time: model!.time - model!.startTime, firings: model!.firings)
        let dl = DataLine(eventType: "trial-end", time: model!.time - model!.startTime, goalBaselevelActivation: slot4!, reachGoal: model!.tasks[model!.currentTaskIndex!].goalHistory.last!!, retrievedOperators: model!.retrievedOp.joined(separator: "-"))
        model!.retrievedOp = []
        model!.outputData.append(dl)
        model!.firings = 0
    }
...
//    let dl = DataLine(eventType: "data-line", eventParameter1: eventParams[0], eventParameter2: eventParams[1], eventParameter3: eventParams[2], inputParameters: model!.scenario.inputMappingForTrace, time: model!.time - model!.startTime, firings: model!.firings)
    let dl = DataLine(eventType: "data-line", time: model!.time - model!.startTime, goalBaselevelActivation: slot4!, reachGoal: false, retrievedOperators: "void")
    model!.outputData.append(dl)
    model!.firings = 0
    return(nil, true)
}
...
func runRelativeTimeOrAction(_ content: [Factor], model: Model?) throws -> (result: Factor?, done: Bool) {
    ...
    model!.newStep()
    //Return array with the sequence of actions
    if let action = model!.buffers["action"] {
        var i = 1
        while (action.slotvals["slot\(i)"] != nil) {
             model!.actionHistory.append(Expression(preop: "", firstTerm: Term(factor: Factor.str(action.slotvals["slot\(i)"]!.description), op: "", term: nil), op: "", secondTerm: nil))
            i += 1
        }
    }
//    else {
//        model!.actionHistory.append(generateFactorExpression(Factor.str("")))
//    }
...
func issueReward(_ content: [Factor], model: Model?) throws -> (result: Factor?, done: Bool) {
    ...
    if model!.dm.goalSelection && model!.batchMode {
        model!.tasks[model!.currentTaskIndex!].updateGoalHistory(model: model!, reward: reward)
    }
    return (nil, true)
}
...
/*
 Updates history of actions of the model (var actionHistory)
 */
func actionHistory(content: [Factor], model: Model?) throws -> (result: Factor?, done: Bool) {
    return (Factor.arr(ScriptArray(elements: model!.actionHistory)), true)
}
