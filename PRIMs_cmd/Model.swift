...
//    var firings: Int = 0
    var time: Double
    var goalBaselevelActivation: Double
    var reachGoal: Bool
    var retrievedOperators: String
...
//    init(eventType: String, eventParameter1: String, eventParameter2: String, eventParameter3: String, inputParameters: [String], time: Double, firings: Int) {
    init(eventType: String, time: Double, goalBaselevelActivation: Double, reachGoal: Bool, retrievedOperators: String) {
...
//        self.firings = firings
        self.goalBaselevelActivation = goalBaselevelActivation
        self.reachGoal = reachGoal
        self.retrievedOperators = retrievedOperators  }
}
...
//                println("Posted Action notification")
    ...
    let silent: Bool
    var actionHistory: [Expression] = []
    var retrievedOp: [String] = []
...
//    struct Results {
    ...
    init(silent: Bool, batchMode: Bool, goalSelection: Bool) {
        trace = []
        self.silent = silent
    ...
    required convenience init?(coder aDecoder: NSCoder) {
        ...
        self.init(silent: false, batchMode: false, goalSelection: false)
        self.dm = dm
    ...
    convenience init(silent: Bool) {
        self.init(silent: silent, batchMode: false, goalSelection: false)
    ...
    convenience init(batchMode: Bool, goalSelection: Bool) {
        self.init(silent: true, batchMode: batchMode, goalSelection: goalSelection)
        self.dm = Declarative(model: self)
        self.procedural = Procedural(model: self)
        self.imaginal = Imaginal(model: self)
        self.action = Action(model: self)
...
//            let slot3 = result!.slotvals["slot3"]?.description
            ...
            var slot4: Double?
            if let goal = buffers["goal"] {
            for (_,value) in goal.slotvals {
                switch value {
                case .symbol(let valchunk):
                    slot4 = valchunk.baseLevelActivation()
                default: break
                    }
                }
            }
//            let dl = DataLine(eventType: "perception", eventParameter1: slot1 ?? "void", eventParameter2: slot2 ?? "void", eventParameter3: slot3 ?? "void", inputParameters: scenario.inputMappingForTrace, time: inputTime - startTime, firings)
            let dl = DataLine(eventType: "perception", time: inputTime - startTime, goalBaselevelActivation: slot4!, reachGoal: false, retrievedOperators: "void" )
...
//                    let dl = DataLine(eventType: "trial-end", eventParameter1: "fail", eventParameter2: "void", eventParameter3: "void", inputParameters: scenario.inputMappingForTrace, time: time - startTime, firings: firings)
                        let dl = DataLine(eventType: "trial-end", time: time - startTime, goalBaselevelActivation: slot4!, reachGoal:  tasks[currentTaskIndex!].goalHistory.last!!, retrievedOperators: retrievedOp.joined(separator: "-"))
                        outputData.append(dl)
                    }
                    firings = 0
                    return
...
//            let dl = DataLine(eventType: "operator", eventParameter1: buffers["goal"]!.slotvals["last-operator"]!.description, eventParameter2: "void", eventParameter3: "void", inputParameters: scenario.inputMappingForTrace, time: time - startTime, firings: firing)
//            outputData.append(dl)
            firings = 0
        }
        commitToTrace(false)
